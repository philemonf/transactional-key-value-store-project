...
public enum LockingUnit
  {
     ...
     * For simplicity, please call this method before running the threads.
     *
     * @param table the lock compatibility table - if null, use default
     * parameter
     */
     ...
     */
    public <T extends LockType> void promote(int transactionID, Serializable key, LockType newType)
      {
        try
          {
            internalLock.lock();

            //TODO get oldLockTypes
            List<T> oldTypes = null;
     ...
     * @param lockType the lock type, be careful to init the module with the
...
//    }
    public void releaseAll(int transactionID)
      {
        internalLock.lock();
        //TODO get current locks for transaction
//         List<key,LockType> locksForTransaction = null;
//         for (somekey,someLock: locksForTransaction)
//           {
//             removeLock(someKey,someLock)
//        signalOn(someKey,someLock)
//           }
        internalLock.unlock();
      }
