...
public class TMWorker extends Thread {
	...
	public void run() {
		try {
			...
			String requestType = jsonRequest
					.getString(JSONCommunication.KEY_FOR_MESSAGE_TYPE);

			switch (requestType) {
			case BeginRequest.MESSAGE_TYPE:
				BeginRequest beginRequest = (BeginRequest) JSON2MessageConverter
						.parseJSON(jsonRequest, BeginRequest.class);
				response = toJSON(concurrencyController.begin(beginRequest));
				break;
			case ReadRequest.MESSAGE_TYPE:
				ReadRequest readRequest = (ReadRequest) JSON2MessageConverter
						.parseJSON(jsonRequest, ReadRequest.class);
				response = toJSON(concurrencyController.read(readRequest));
				break;
			case WriteRequest.MESSAGE_TYPE:
				WriteRequest writeRequest = (WriteRequest) JSON2MessageConverter
						.parseJSON(jsonRequest, WriteRequest.class);
				response = toJSON(concurrencyController.write(writeRequest));
				break;
			case CommitRequest.MESSAGE_TYPE:
				CommitRequest commitRequest = (CommitRequest) JSON2MessageConverter
						.parseJSON(jsonRequest, CommitRequest.class);
