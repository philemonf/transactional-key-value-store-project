...
public interface IVersioningUnit {
    ...
    /**
     * Commit the changes done by a transaction
     * The transaction SHOULD NOT do any other requests to the VersioningUnit
     ...
     * The transaction SHOULD NOT do any other requests
     * @param xid the transaction (= Xact timestamp) to be aborted
     */
    public void abort(int xid);
    
    /**
     * Stop gracefully the VersioningUnit 
     * Must be call if one wants to restart the VersioningUnit No need to call it if the application is exiting
     */
    public void stopNow();
